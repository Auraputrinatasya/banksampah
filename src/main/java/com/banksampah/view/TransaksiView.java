...
public class TransaksiView {
    ...
    public TransaksiView() { buildUI(); }

    private boolean isAdmin() { return SessionManager.isAdmin(); }
    ...
    private void buildUI() {
        ...
        topbar.getChildren().addAll(title, spacer, btnTambah, new Region() {{ setMinWidth(8); }}, btnRefresh);
        ...
        colAksi.setCellFactory(col -> new TableCell<>() {
            ...
            {
                btnEdit.setStyle("-fx-background-color:#3B82F6;-fx-text-fill:white;-fx-cursor:hand;-fx-padding:4 10;-fx-background-radius:5;-fx-font-size:12px;");
                btnHapus.setStyle("-fx-background-color:#EF4444;-fx-text-fill:white;-fx-cursor:hand;-fx-padding:4 10;-fx-background-radius:5;-fx-font-size:12px;");
                btnVerif.setStyle("-fx-background-color:#16A34A;-fx-text-fill:white;-fx-cursor:hand;-fx-padding:4 10;-fx-background-radius:5;-fx-font-size:12px;");
                btnPrint.setStyle("-fx-background-color:#8B5CF6;-fx-text-fill:white;-fx-cursor:hand;-fx-padding:4 10;-fx-background-radius:5;-fx-font-size:12px;");
            }
            @Override protected void updateItem(String item, boolean empty) {
                super.updateItem(item, empty);
                ...
                if (!sudahDiproses || isAdmin()) {
                    ...
                    if (sudahDiproses)
                        btnHapus.setStyle("-fx-background-color:#9CA3AF;-fx-text-fill:white;-fx-cursor:hand;-fx-padding:4 10;-fx-background-radius:5;-fx-font-size:12px;");
                    else
                        btnHapus.setStyle("-fx-background-color:#EF4444;-fx-text-fill:white;-fx-cursor:hand;-fx-padding:4 10;-fx-background-radius:5;-fx-font-size:12px;");
                ...
                btnPrint.setOnAction(e -> {
                    try { Desktop.getDesktop().browse(new URI(ReportServer.getDetailUrl(s.getIdSetor()))); }
    ...
    private void handleHapus(SetorSampah s) {
        if (!isAdmin() && ("diterima".equals(s.getStatus()) || "ditolak".equals(s.getStatus()))) {
            showAlert("Tidak Bisa Dihapus", "Transaksi yang sudah " + s.getStatus() + " tidak dapat dihapus!"); return;
        }
        ...
        conf.showAndWait().ifPresent(b -> {
            if (b == ButtonType.YES) {
                try { dao.delete(s.getIdSetor()); loadData(); }
                catch (Exception e) { showAlert("Error", e.getMessage()); }
            }
        });
    }
    ...
    private void handleVerifikasi(SetorSampah s) {
        if (!isAdmin()) { showAlert("Akses Ditolak", "Hanya Admin yang bisa memverifikasi!"); return; }
        ...
        dlg.showAndWait().ifPresent(b -> {
            try {
                if (b == terima) dao.updateStatus(s.getIdSetor(), "diterima", SessionManager.getCurrentUser().getIdUser());
                else if (b == tolak) dao.updateStatus(s.getIdSetor(), "ditolak", SessionManager.getCurrentUser().getIdUser());
                loadData();
            } catch (Exception e) { showAlert("Error", e.getMessage()); }
        });
    }
    ...
    private void showFormDialog(SetorSampah existing) {
        if (existing != null) {
            ...
            if (sudah) { showAlert("Tidak Bisa Diedit", "Transaksi yang sudah " + existing.getStatus() + " tidak dapat diedit!"); return; }
        }
        ...
        if (isAdmin()) {
            try { cmbNasabah.setItems(FXCollections.observableArrayList(new UserDAO().getNasabah())); }
        ...
        TextField txtCatatan = new TextField(); txtCatatan.setPromptText("Catatan (opsional)");
        ...
        try { cmbJenis.setItems(FXCollections.observableArrayList(new JenisSampahDAO().getAll())); }
        catch (Exception e) { e.printStackTrace(); }

        TextField txtBerat = new TextField(); txtBerat.setPromptText("Berat (kg)");
        ...
        btnAdd.setOnAction(e -> {
            ...
            if (j == null || txtBerat.getText().isEmpty()) return;
            try {
                double berat = Double.parseDouble(txtBerat.getText());
                double subtotal = berat * j.getHargaPerKg();
                DetailSetor d = new DetailSetor();
                d.setIdJenis(j.getIdJenis()); d.setBerat(berat);
                d.setHargaSatuan(j.getHargaPerKg()); d.setSubtotal(subtotal); d.setNamaJenis(j.getNamaJenis());
                details.add(d);
                listItems.getItems().add(j.getNamaJenis() + " | " + berat + " kg | Rp " + IDR.format((long) subtotal));
                txtBerat.clear();
            } catch (NumberFormatException ex) { showAlert("Error", "Berat harus berupa angka!"); }
        });

        if (existing != null) {
            if (isAdmin()) {
                for (User u : cmbNasabah.getItems())
                    if (u.getIdUser() == existing.getIdUser()) { cmbNasabah.setValue(u); break; }
            }
            ...
            try {
                for (DetailSetor d : dao.getDetail(existing.getIdSetor())) {
                    details.add(d);
                    listItems.getItems().add(d.getNamaJenis() + " | " + d.getBerat() + " kg | Rp " + IDR.format((long) d.getSubtotal()));
                }
            } catch (Exception e) { e.printStackTrace(); }
        }
        ...
        btnSimpan.setOnAction(e -> {
            if (cmbNasabah.getValue() == null) { showAlert("Validasi", "Pilih nasabah!"); return; }
            if (details.isEmpty()) { showAlert("Validasi", "Minimal tambah 1 item sampah!"); return; }
            try {
                ...
                int idSetor;
                if (existing == null) { s.setStatus("menunggu"); idSetor = dao.insert(s); }
                else { idSetor = existing.getIdSetor(); dao.update(s); dao.deleteDetail(idSetor); }
                for (DetailSetor d : details) { d.setIdSetor(idSetor); dao.insertDetail(d); }
